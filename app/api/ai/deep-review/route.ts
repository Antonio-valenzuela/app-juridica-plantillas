...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateLimitResult = checkRateLimit(ip, 15); // Rate limit for deep review
    if (!rateLimitResult.ok) {
      return NextResponse.json(
        { ok: false, error: "rate_limit", friendlyMessage: "Demasiadas solicitudes de revisión profunda. Por favor intente más tarde." },
        { status: 429 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error.message || "Error al procesar la revisión profunda." },
      { status: 500 }
    );
  }
}
